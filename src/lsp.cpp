...
static std::string handleHover(int id, yyjson_val* params) {
    ...
    static const std::unordered_map<std::string, std::string> docs = {
        ...
        {"each", "`(each var collection body...)` — iterate values"},
        {"fn", "`(fn (params...) body...)` — define function/closure"},
        {"do", "`(do body...)` — sequential block, returns last value"},
        {"with", "`(with var init body...)` — RAII: calls destructor on exit"},
        ...
        {"and", "`(and a b)` — short-circuit logical AND (returns a if falsy, else b)"},
        {"or", "`(or a b)` — short-circuit logical OR (returns a if truthy, else b)"},
        {"not", "`(not val)` — logical negation"},
        {"vector", "`(vector type vals...)` — typed contiguous array"},
        ...
        {"execute", "`(execute cmd)` — run shell command, return exit code"},
        {"spawn", "`(spawn cmd)` — run command, return process handle with :read/:wait"},
        {"spawn-detached", "`(spawn-detached cmd)` — run background process, return PID"},
